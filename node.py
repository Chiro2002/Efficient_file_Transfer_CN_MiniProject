...
class Node:
    ...
    def split_owners(self, filename: str, owners: list):
        ...
        for i, o in enumerate(owners):
            t = Thread(target=self.receive_file,
                       args=(filename, ranges[i], o),daemon=True)
            t.start()
            threads.append(t)
    ...
    def set_upload(self, filename: str):
        ...
        t = Thread(target=self.start_listening, args=(),daemon=True)
        t.start()
...
def main(name: str, rec_port: int, send_port: int):
    ...
    while True:
        if "upload" in command:
            ...
            filename = command.split(' ')[3]
            t2 = Thread(target=node.start_download, args=(filename,),daemon=True)
            t2.start()
